...
class OperationData {
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (status, price_in,min_price,max_price,price_out,stock_id,product_id,q,operation_type_id,sell_id,user_id,created_at) ";
		$sql .= "values ($this->status, $this->price_in,$this->min_price,$this->max_price,$this->price_out,$this->stock_id,\"$this->product_id\",\"$this->q\",$this->operation_type_id,\"$this->sell_id\",$this->user_id,$this->created_at)";
		return Executor::doit($sql);
	}

	public function add_issell(){
		$sql = "insert into ".self::$tablename." (status, price_in,price_out,stock_id,product_id,q,operation_type_id,sell_id,is_sell,user_id,created_at) ";
		$sql .= "values ($this->status, $this->price_in,$this->price_out,$this->stock_id,\"$this->product_id\",\"$this->q\",$this->operation_type_id,$this->sell_id,$this->is_sell,$this->user_id,$this->created_at)";
		return Executor::doit($sql);
	}

	public function add_cotization(){
		$sql = "insert into ".self::$tablename." (price_in,min_price,max_price,price_out,is_draft,stock_id,product_id,q,operation_type_id,sell_id,user_id,created_at) ";
		$sql .= "values ($this->price_in,0,0,$this->price_out,1,	$this->stock_id,\"$this->product_id\",\"$this->q\",$this->operation_type_id,\"$this->sell_id\",$this->user_id,$this->created_at)";
		return Executor::doit($sql);
	}
	...
	public function update(){
		$sql = "update ".self::$tablename." set product_id=\"$this->product_id\",q=\"$this->q\",user_id=$this->user_id where id=$this->id";
		Executor::doit($sql);
	}

	public function set_draft($d){
		$sql = "update ".self::$tablename." set is_draft=\"$d\",user_id=$this->user_id where id=$this->id";
		Executor::doit($sql);
	}

	public function update_q(){
		$sql = "update ".self::$tablename." set q=\"$this->q\",user_id=$this->user_id where id=$this->id";
		Executor::doit($sql);
	}

	public function cancel(){
		$sql = "update ".self::$tablename." set status=0,user_id=$this->user_id where id=$this->id";
		Executor::doit($sql);
	}

	public function update_type(){
		$sql = "update ".self::$tablename." set operation_type_id=\"$this->operation_type_id\",user_id=$this->user_id where id=$this->id";
		Executor::doit($sql);
	}

	public function update_status(){
		$sql = "update ".self::$tablename." set status=$this->status,user_id=$this->user_id where id=$this->id";
		Executor::doit($sql);
	}
    ...
    public static function getByStockById($id){
		$sql = "SELECT *FROM ".self::$tablename." where product_id = $id and operation_type_id = 1 and stock_id = (SELECT MIN(stock_id) FROM ".self::$tablename." WHERE product_id = $id)";
		$query = Executor::doit($sql);
		return Model::one($query[0],new OperationData());
	}
	...
	public static function getAllQ($start, $end){
		$sql = "select SUM(q) as suma from ".self::$tablename." where (date(created_at) >= \"$start\" and date(created_at) <= \"$end\") and operation_type_id=2 order by created_at";
		$query = Executor::doit($sql);
		return Model::one($query[0],new OperationData());
	...
	public static function getAllByDateOfficial($stock,$start,$end){
	 ...
	 inner join product on operation.product_id = product.id where (date(".self::$tablename.".created_at) >= \"$start\"
	 and date(".self::$tablename.".created_at) <= \"$end\") and ".self::$tablename.".stock_id=$stock order by ".self::$tablename.".created_at desc";
		$query = Executor::doit($sql);
		return Model::many($query[0],new OperationData());
	}

	public static function getAllByDateOfficialReport($stock,$start,$end){
        $sql = "select ".self::$tablename.".* from ".self::$tablename.
        " inner join product on operation.product_id = product.id where (date(".self::$tablename.".created_at)) >= \"$start\" and date(".self::$tablename.".created_at) <= \"$end\" and operation.stock_id = 1  order by ".self::$tablename.".created_at desc";
		$query = Executor::doit($sql);
		return Model::many($query[0],new OperationData());
	}

	public static function getAllOfficial(){
        $sql = "select ".self::$tablename.".*,product.barcode,product.name,status.description status_dsc from ".self::$tablename." inner join product on operation.product_id = product.id INNER JOIN status ON product.status=status.id order by ".self::$tablename.".created_at desc";
		$query = Executor::doit($sql);
		return Model::many($query[0],new OperationData());
	}
    ...
    public static function getAllOfficialBP($stock,$product, $start,$end){
	...
	public function getProduct(){ return ProductData::getProductServiceById($this->product_id);}
	...
	public static function getQByStock($product_id){
		$q=0;
		$operations = self::getAllByProductIdAndStock($product_id);
		$input_id = OperationTypeData::getByName("entrada")->id;
		$output_id = OperationTypeData::getByName("salida")->id;
		foreach($operations as $operation){
            if($operation->operation_type_id==$input_id){ $q+=$operation->q; }
	...
	public static function getAllByProductIdAndStock($product_id){
		$sql = "select * from ".self::$tablename." where product_id=$product_id and is_draft=0 and status=1 order by created_at desc";
		$query = Executor::doit($sql);
		return Model::many($query[0],new OperationData());
	}
	...
	public static function getAllProductsBySellId($sell_id){
		$sql = "select * from ".self::$tablename." where sell_id=$sell_id and is_sell = 1 order by created_at desc";
		$query = Executor::doit($sql);
		return Model::many($query[0],new OperationData());
	}

	public static function getAllProductsByRefId($sell_id){
		$sql = "select * from ".self::$tablename." where sell_id=\"$sell_id\" and is_sell = 1 order by created_at desc";
		$query = Executor::doit($sql);
		return Model::many($query[0],new OperationData());
	}
