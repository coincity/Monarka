...
$pdf->Cell(5,55,'---------------------------------------------------------------------------------------------------------------------');
$pdf->setX(2);
$pdf->Cell(5,58,utf8_decode('ARTÍCULO                             CANT.    PRECIO                                          TOTAL      '));
$pdf->setX(2);
$pdf->Cell(5,61,'---------------------------------------------------------------------------------------------------------------------');
...
foreach($operations as $op){
    ...
    $pdf->setX(45);
    $itbis = 0;
    if($product->itbis == 1) $itbis = ($op->price_out *($sell->iva/100));
    $pdf->setX(60);
    $pdf->Cell(11,$off,  $currency.number_format(($op->q*$op->price_out),2,".",",") ,0,0,"L");
...
$pdf->setX($values_x);
$pdf->Cell(5,$off,Utils::moneyFormat($currency, $sell->subtotal),0,0,"R");
...
$pdf->setX($values_x);
$pdf->Cell(5,$off,Utils::moneyFormat($currency, $sell->taxes),0,0,"R");
...
$pdf->setX($values_x);
$pdf->Cell(5,$off, $currency." ".number_format($sell->total,2,".",","),0,0,"R");
$pdf->setX($details_x);
$off+=6;
...
$pdf->setX($mid_x + 4 - ($pdf->GetStringWidth("") / 2));
$off+8;
$pdf->Cell(5,$off,"");
$pdf->setX(2);
$off+=5;
