...
$operations = array();
			...
			if($_GET["product_id"]==""){
				if($_GET["stock_id"]!=""){
					...
					$operations = OperationData::getAllOfficial($_GET["stock_id"],$_GET["sd"],$_GET["ed"]);
				}
			}
			else{
				if($_GET["stock_id"]!=""){
					...
					$operations = OperationData::getAllOfficialBP($_GET["stock_id"],$_GET["product_id"],$_GET["sd"],$_GET["ed"]);
				}
			}
...
$objPHPExcel->getProperties()->setCreator("SySpa 3.0")
							 ...
							 ->setTitle("Report - SySpa 3.0")
							 ->setSubject("SySpa Report")
							 ->setDescription("")
							 ->setKeywords("")
							 ->setCategory("");
...
$sheet->setCellValue('A1', 'Reporte de Inventario - SySpa')
->setCellValue('A2', 'Id')
